...
import feature_extractors.cnn.predictor as cnn_predicor
...
class EvaluateAll:
    ...
    def run_evaluation(self):
        ...
        for im_name in im_list:
            ...
            cnn = cnn_predicor.CNN(model='model_opt2.h5')

            scores, best_class = cnn.predict(img)
            score_arr.append(np.array(scores).reshape(1, -1))

        Y_plain = cdist(plain_features_arr, plain_features_arr, 'jensenshannon')
        
        r1 = eval.compute_rank1(Y_plain, y)
        print('pix-2-pix-Rank-1', r1)

        r5 = eval.compute_rank5_accuracy(Y_plain, y)
        print('\npix-2-pix-Rank-5', r5)

        r1_CNN = eval.compute_rank1_accuracy(score_arr, y)
        print('\ncnn-Rank-1', r1_CNN)
        
        r5_CNN = eval.compute_rank5_accuracy(score_arr, y)
        print('\ncnn-Rank-5', r5_CNN)
        
        eval.plotCMC(np.array(score_arr), np.array(y))
